...
class DoctorServices():
	def __init__(self, db_ ):
		...
		self.query: orm.Query = self.db.query(models.DoctorDB)
	...
	def read_by_username(self, username):
		try:
			return self.query.filter(models.DoctorDB.username == username).one()
		except NoResultFound as exc:
			abort(make_response(jsonify(message="Username or Password is invalid"), 400))

	def read_by_id(self, id_):
		try:
			return self.query.filter(models.DoctorDB.id == id_).one()
		except NoResultFound as exc:
			abort(make_response(jsonify(message=F"Doctor with id {id_} is not found"), 404))

	def create(self, kwargs):
		kwargs.password = sha256_crypt.encrypt(kwargs.password)
		try:
			db_data = models.DoctorDB(**kwargs.dict())
			self.db.add(db_data)
			self.db.commit()
			return db_data
	...
	def put_data(self, id_, new_data):
		...
		if new_data.work_start_time is not None:
			db_data.work_start_time = new_data.work_start_time
		if new_data.work_end_time is not None:
			db_data.work_end_time = new_data.work_end_time
	...
	def delete(self, id_):
		...
		self.query.filter(models.DoctorDB.id == id_).delete()
